...
#  CreateShortCut "$DESKTOP\Akonadiconsole.lnk" "$INSTDIR\bin\akonadiconsole.exe"
  ...
  CreateDirectory "$APPDATA\.kontact"
  CreateDirectory "$APPDATA\.kontact\.local"
  CreateDirectory "$APPDATA\.kontact\.local\share"
  CreateDirectory "$APPDATA\.kontact\.local\share\akonadi"
  CreateDirectory "$APPDATA\.kontact\.local\share\akonadi\db_data"

  ; Create startmysql.bat 
  FileOpen $1 "$INSTDIR\bin\startmysql.bat" "w"
  FileWrite $1 '@echo off $\r$\n'
  FileWrite $1 'if not exist "%APPDATA%\.kontact\.local\\share\akonadi\db_data" mkdir "%APPDATA%\.kontact\.local\\share\akonadi\db_data" $\r$\n'
  FileWrite $1 'START /B "AkonadiMysqld" "$INSTDIR\bin\mysqld.exe" --defaults-file="$INSTDIR\\share\\config\\akonadi\\mysql-global.conf" --datadir=%APPDATA%\\.kontact\\.local\\share\\akonadi\\db_data\\ --shared-memory'
  FileClose $1
